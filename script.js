...
document.getElementById('totalTime').textContent = "Время на задачу: " + days + " д " + hours + " ч " + minutes + " мин";
}
...
function clearAll() {
  ...
  fields.forEach(id => {
    ...
    input.value = 0;
    input.classList.remove('selected'); // Удаляем класс, чтобы убрать чёрную обводку
  });
...
document.addEventListener('DOMContentLoaded', function() {
  // Для всех полей, где type="number", добавляем «слушатели»
